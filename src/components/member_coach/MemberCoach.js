...
function Component() {
  ...
  return (
    <div>
      ...
      />
      <div>
        <button onClick={()=>{setTarget(2); setUpdate(Date())}}>manage coaches</button>
        <button onClick={()=>{setTarget(1); setUpdate(Date())}}>manage members</button>
      </div>
      { 
        selectedGym && target
        ? <div>
            <input value={email} onChange={e=>{setEmail(e.target.value)}}></input>
            <button type='email' onClick={()=>{Functions.addMember(selectedGym.id, email, setUpdate, setAuth)}}>Add {target == 1 ? 'member' : 'coach'}</button>
            <br/>
            { 
              ...
              ? <>
                ...
                {selectedGymUsers.map((user, index)=>
                  <div key={index}>
                    ...
                    <button onClick={()=>{Functions.deleteUserByGym(user.id, selectedGym.id, setUpdate, setAuth)}}>delete</button>
                  </div>
                )}
              </>
              : <></>
